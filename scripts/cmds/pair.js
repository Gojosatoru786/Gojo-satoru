...
module.exports = {
  ...
  onStart: async function ({ api, event, usersData }) {
    ...
    for (let id of listUserID) {
      ...
      if ((senderGender === 0 && userGender !== 0) || (senderGender === 1 && userGender !== 1) || (senderGender === 2 && userGender !== 2)) {
        possibleIDs.push(id);
      }
    }
    if (possibleIDs.length === 0) {
      return api.sendMessage("No possible pairs found.", event.threadID, event.messageID);
    }
